...
#' This method is dispatched when \code{stan_engine = "cmdstan-optimize"} in 
...
#' @param ... Additional arguments passed to \code{\link[cmdstanr]{optimize}}.
#' @return A cmdstan_optimize_analysis object.
#' @keywords internal
#' @export
analyse1.cmdstan_optimize_model <- function(model, data, loaded, nchains, niters, nthin, 
  ...
  capture_output(
    cmdstan_fit <- do.call(loaded$optimize, c(list(
      data = data,
      seed = seed,
      init = init,
      show_messages = !quiet,
  ...
  class(obj) <- c("cmdstan_optimize_analysis", "cmdstan_analysis", "mb_analysis")
